    ...
    return new Promise((resolve)=>{
        ...
        const server = app.listen(0,async ()=>{
           ...
           const itemData = await itemRes.json();
            console.log(`GET /item`,JSON.stringify(itemData))
...
function block_2_response(){
    return new Promise((resolve)=>{
        ...
        app.get('/old-menu',(req,res)=>{
            // add entry in DB to see how many users are still visiting old routes
            res.redirect(301, '/new-menu')
        })

        app.get('/xml',(req,res)=>{
            res.type('application/xml').send('<dish> <name>Biryani</dish>')
        })
